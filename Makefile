...
test: manifests generate fmt vet envtest ## Run tests.
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" go test $$(go list ./... | egrep -v '(test|mocks|e2e/)') -coverprofile coverage.out

.PHONY: test-race
test-race: manifests generate fmt vet envtest ## Run tests with -race option
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" go test -race $$(go list ./... | egrep -v '(test|mocks|e2e/)') -coverprofile coverage.out
...
e2e-tests-parallel-ginkgo: ginkgo
	...
	$(GINKGO_CLI) -p -v -procs=3 --trace --timeout 90m -r ./test/ginkgo/parallel

GINKGO_CLI = $(shell pwd)/bin/ginkgo
.PHONY: ginkgo
ginkgo: ## Download ginkgo locally if necessary.
	$(call go-install-tool,$(GINKGO_CLI),github.com/onsi/ginkgo/v2/ginkgo,v2.32.0)
