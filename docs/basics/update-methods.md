...
To update the <b>Upstream Commit</b> message for different Git providers, following examples can be used:
* **GitLab**:    `Upstream Commit: {{ $source }}/-/commit/{{ $revision }}`
* **BitBucket**:  `Upstream Commit: {{ $source }}/commits/{{ $revision }}`
