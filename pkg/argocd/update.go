...
func marshalWithIndent(in interface{}, indent int) (out []byte, err error) {
	return yaml.MarshalWithOptions(in, yaml.Indent(indent))
}
...
func marshalParamsOverride(ctx context.Context, applicationImages *ApplicationImages, originalData []byte) ([]byte, error) {
	...
	case ApplicationTypeHelm:
		...
		if wbc != nil && !strings.HasPrefix(filepath.Base(wbc.Target), common.DefaultTargetFilePrefix) {
			...
			var helmNewValues *ValuesFile
			emptyOriginalData := isOnlyWhitespace(originalData)
			if emptyOriginalData {
				// allow non-exists target file
				helmNewValues = CreateEmptyValuesFile("auto generated by argocd image updater")
			} else {
				helmNewValues, err = ParseValuesFile(originalData)
				if err != nil {
					return nil, err
				}
			}
			...
			for _, c := range images {
				...
				if helmParamVersion == "" {
					...
					err = helmNewValues.SetValue(helmParamVersion, tagValue)
					if err != nil {
				...
				if helmParamN == nil {
					...
					if !emptyOriginalData && image.HasRegistryPrefix(valueToSet) {
						...
						originalValue, err := helmNewValues.GetValue(helmParamName)
						if err == nil {
						...
						// If GetValue returns an error (key not found), use helmParamN.Value as-is
					}
					// If helmParamN.Value is already in short form or originalData is empty, use it as-is
				}

				err = helmNewValues.SetValue(helmParamName, valueToSet)
				if err != nil {
			...
			newParams := helmOverride{
				Helm: helmParametersYAML{
					Parameters: toHelmParametersYAML(appSource.Helm.Parameters),
				},
			}
			...
			if len(originalData) == 0 {
				sortHelmParametersYAML(newParams.Helm.Parameters)
				override, err = marshalWithIndent(newParams, defaultIndent)
				break
			}
			err = yaml.Unmarshal(originalData, &params)
			...
			sortHelmParametersYAML(params.Helm.Parameters)
			override, err = marshalWithIndent(params, defaultIndent)
		}
	default:
...
func sortHelmParametersYAML(params []helmParameterYAML) {
	slices.SortFunc(params, func(a, b helmParameterYAML) int {
...
func mergeHelmOverride(t *helmOverride, o *helmOverride) {
	for _, param := range o.Helm.Parameters {
		idx := slices.IndexFunc(t.Helm.Parameters, func(tp helmParameterYAML) bool { return tp.Name == param.Name })
		if idx != -1 {
