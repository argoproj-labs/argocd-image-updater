...
func Test_SetHelmImage(t *testing.T) {
	...
	t.Run("Test set Helm image on multi-source app using SourceIndex", func(t *testing.T) {
		app := &v1alpha1.Application{
			ObjectMeta: v1.ObjectMeta{
				Name:      "test-app",
				Namespace: "testns",
	...
	t.Run("Test set Helm image disambiguates duplicate chart names via SourceIndex", func(t *testing.T) {
		app := &v1alpha1.Application{
			ObjectMeta: v1.ObjectMeta{
				Name:      "test-app",
				Namespace: "testns",
		...
		wbc := &WriteBackConfig{Target: "helmvalues:."}
		appImage := &Image{
			HelmSourceIndex: 1, // target the second source, not the first
			HelmImageName:   "image.name",
			HelmImageTag:    "image.tag",
		}
		err := SetHelmImage(context.Background(), app, img, wbc, appImage)
		...
		var idx1Tag v1alpha1.HelmParameter
		for _, p := range app.Spec.Sources[1].Helm.Parameters {
			if p.Name == "image.tag" {
				idx1Tag = p
				break
		...
		assert.Equal(t, "1.0.1", idx1Tag.Value)

		// index 0 should be untouched
		var idx0Tag v1alpha1.HelmParameter
		for _, p := range app.Spec.Sources[0].Helm.Parameters {
			if p.Name == "image.tag" {
				idx0Tag = p
				break
			}
		}
		assert.Equal(t, "1.0.0", idx0Tag.Value)
	})

	t.Run("Test set Helm image with out-of-range SourceIndex returns error", func(t *testing.T) {
		app := &v1alpha1.Application{
			ObjectMeta: v1.ObjectMeta{
				Name:      "test-app",
				Namespace: "testns",
	...
	t.Run("Test SourceIndex takes precedence over ChartName in multi-source app", func(t *testing.T) {
		...
		appImage := &Image{
			HelmSourceIndex: 1,
			HelmChartName:   "my-app", // would match both; SourceIndex should win and pick index 1
			HelmImageName:   "image.name",
			HelmImageTag:    "image.tag",
		}
		err := SetHelmImage(context.Background(), app, img, wbc, appImage)
		...
		// index 0 should be untouched
		assert.Empty(t, app.Spec.Sources[0].Helm.Parameters)
	})
...
func Test_newImageFromManifestTargetSettings(t *testing.T) {
	...
	t.Run("should populate HelmSourceIndex from settings", func(t *testing.T) {
		idx := int32(2)
		settings := &api.ManifestTarget{
			Helm: &api.HelmTarget{
				SourceIndex: &idx,
		...
		img, err := newImageFromManifestTargetSettings(settings, &Image{HelmSourceIndex: -1})
		assert.NoError(t, err)
		assert.Equal(t, 2, img.HelmSourceIndex)
	})

	t.Run("should not overwrite HelmSourceIndex when settings has no SourceIndex", func(t *testing.T) {
		settings := &api.ManifestTarget{
			Helm: &api.HelmTarget{
				Name: strPtr("image.name"),
			},
		...
		img, err := newImageFromManifestTargetSettings(settings, &Image{HelmSourceIndex: 1})
		assert.NoError(t, err)
		assert.Equal(t, 1, img.HelmSourceIndex)
	})
...
func Test_FilterApplicationsForUpdate(t *testing.T) {
	...
	testCases := []struct {
		...
		// TODO: Temporary tests for spec.namespace behavior. Remove these tests when spec.namespace is completely removed from the API.
		{
			name: "spec.namespace equals metadata.namespace - should work",
			initialApps: []client.Object{
				&v1alpha1.Application{
					ObjectMeta: v1.ObjectMeta{Name: "app-in-testns", Namespace: "testns"},
					Spec: v1alpha1.ApplicationSpec{
						Source: &v1alpha1.ApplicationSource{
							RepoURL:        "https://github.com/example/repo.git",
							TargetRevision: "main",
			...
			imageUpdaterCR: &api.ImageUpdater{
				ObjectMeta: v1.ObjectMeta{Name: "test-updater", Namespace: "testns"},
				Spec: api.ImageUpdaterSpec{
					Namespace: stringPtr("testns"), // spec.namespace equals metadata.namespace
					ApplicationRefs: []api.ApplicationRef{
						{NamePattern: "app-in-testns", Images: []api.ImageConfig{{Alias: "nginx", ImageName: "nginx:1.0"}}},
					},
				},
			},
			expectedKeys: []string{"testns/app-in-testns"},
		},
		{
			name: "spec.namespace differs from metadata.namespace - should ignore spec.namespace and use metadata.namespace",
			initialApps: []client.Object{
				&v1alpha1.Application{
					ObjectMeta: v1.ObjectMeta{Name: "app-in-testns", Namespace: "testns"},
					Spec: v1alpha1.ApplicationSpec{
				...
				&v1alpha1.Application{
					ObjectMeta: v1.ObjectMeta{Name: "app-in-otherns", Namespace: "otherns"},
					Spec: v1alpha1.ApplicationSpec{
						Source: &v1alpha1.ApplicationSource{
							RepoURL:        "https://github.com/example/repo.git",
							TargetRevision: "main",
			...
			imageUpdaterCR: &api.ImageUpdater{
				ObjectMeta: v1.ObjectMeta{Name: "test-updater", Namespace: "testns"},
				Spec: api.ImageUpdaterSpec{
					Namespace: stringPtr("otherns"), // spec.namespace differs from metadata.namespace (testns)
					ApplicationRefs: []api.ApplicationRef{
						{NamePattern: "*", Images: []api.ImageConfig{{Alias: "nginx", ImageName: "nginx:1.0"}}},
					},
				},
			},
			// Should only find app in testns (metadata.namespace), not in otherns (spec.namespace)
			expectedKeys: []string{"testns/app-in-testns"},
		},
		{
			name: "spec.namespace differs from metadata.namespace - no apps in metadata.namespace should return nil",
			initialApps: []client.Object{
				&v1alpha1.Application{
					ObjectMeta: v1.ObjectMeta{Name: "app-in-otherns", Namespace: "otherns"},
					Spec: v1alpha1.ApplicationSpec{
			...
			// Should return nil because controller uses metadata.namespace (testns) where no apps exist
			expectNilResult: true,
		},
	}
