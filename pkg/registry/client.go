...
func ping(manager challenge.Manager, endpoint *RegistryEndpoint, versionHeader string) ([]auth.APIVersion, error) {
	...
	endpointURL := endpoint.RegistryAPI + "/v2/"
	resp, err := httpc.Get(endpointURL)
	if err != nil {
		return nil, err
	}
	...
	if resp.StatusCode != 200 && resp.StatusCode != 401 {
		return nil, fmt.Errorf("endpoint %s does not seem to be a valid v2 Docker Registry API (received HTTP code %d for GET %s)", endpoint.RegistryAPI, resp.StatusCode, endpointURL)
	}
