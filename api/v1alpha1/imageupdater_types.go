...
type ImageUpdaterSpec struct {
	...
	// --- Defaults/Global settings for all applications matched by this CR ---
	...
	// IgnoreTags is a list of tags to ignore.
	...
	// WriteBackMethod defines the method for writing back updated image versions.
	// This acts as the default if not overridden.
	// +optional
	// +kubebuilder:default:="argocd"
	WriteBackMethod *string `json:"writeBackMethod,omitempty"`
	...
	// +kubebuilder:validation:MinItems=1
	Images []ImageConfig `json:"images"` // Assuming this list is mandatory if the CR is to do anything.
}
...
type ApplicationRef struct {
	...
	// UpdateStrategy overrides the spec-level setting for applications matched by this selector.
	// +optional
	UpdateStrategy *string `json:"updateStrategy,omitempty"`

	// ForceUpdate overrides the spec-level setting for applications matched by this selector.
	// +optional
	ForceUpdate *bool `json:"forceUpdate,omitempty"`

	// AllowTags overrides the spec-level setting for applications matched by this selector.
	// +optional
	AllowTags *string `json:"allowTags,omitempty"`

	// IgnoreTags overrides with the spec-level setting for applications matched by this selector.
	// +listType=atomic
	// +optional
	IgnoreTags []string `json:"ignoreTags,omitempty"`

	// PullSecret overrides the spec-level setting for applications matched by this selector.
	// +optional
	PullSecret *string `json:"pullSecret,omitempty"`

	// WriteBackMethod overrides the spec-level setting for applications matched by this selector.
	// +optional
	WriteBackMethod *string `json:"writeBackMethod,omitempty"`

	// GitConfig overrides the spec-level Git configuration for applications matched by this selector.
	// +optional
	GitConfig *GitConfig `json:"gitConfig,omitempty"`
}
...
type ImageConfig struct {
	...
	// UpdateStrategy overrides the default update strategy for this specific image.
	// +optional
	UpdateStrategy *string `json:"updateStrategy,omitempty"`

	// ForceUpdate overrides the default force update behavior for this specific image.
	// +optional
	ForceUpdate *bool `json:"forceUpdate,omitempty"`

	// AllowTags overrides the default allow tags regex for this specific image.
	// +optional
	AllowTags *string `json:"allowTags,omitempty"`

	// IgnoreTags overrides or merges with the default ignore tags list for this specific image.
	// +listType=atomic
	// +optional
	IgnoreTags []string `json:"ignoreTags,omitempty"`

	// PullSecret overrides the default pull secret for this specific image.
	// +optional
	PullSecret *string `json:"pullSecret,omitempty"`
	...
	// ManifestTargets defines how and where to update this image in Kubernetes manifests.
	// Only one of Helm or Kustomize should be specified within this block.
	// +optional // This whole block is optional if the image update isn't written to a manifest in a structured way.
	ManifestTargets *ManifestTarget `json:"manifestTargets,omitempty"`
}
